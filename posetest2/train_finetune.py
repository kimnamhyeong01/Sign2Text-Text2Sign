...
BATCH_SIZE = 32
EPOCHS = 30  # 미세 조정은 적은 epoch로
LR = 1e-5    # 미세 조정용 낮은 learning rate
...
# 모델 불러오기 (이전 학습 모델 이어서)
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
model = PoseFormer(num_classes=NUM_CLASSES).to(device)
model.load_state_dict(torch.load("E:/pose/pose_transformer_full.pt"))
...
# TTA 함수
def tta_predict(model, x, n_augment=5):
    preds = []
...
for epoch in range(EPOCHS):
    ...
    with torch.no_grad():
        for x, y in val_loader:
            x, y = x.to(device), y.to(device)
            output = tta_predict(model, x)  # TTA 적용
            loss = criterion(output, y)
            val_loss += loss.item()
    ...
    if val_acc_list[-1] > best_val_acc:
        ...
        best_epoch = epoch + 1
        torch.save(model.state_dict(), "E:/pose/pose_transformer_finetuned.pt")
...
plt.tight_layout()
plt.savefig("E:/pose/finetuning_plot.png")
plt.show()

print(f"\n🎯 Finetuned Best Val Accuracy: {best_val_acc:.4f} at Epoch {best_epoch}")
print("✅ Fine-tuned model saved to: E:/pose/pose_transformer_finetuned.pt")
